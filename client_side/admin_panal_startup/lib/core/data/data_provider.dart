...
// Add these methods to your DataProvider class in data_provider.dart

  void sortCategories(int Function(Category a, Category b) compare) {
    _filteredCategories.sort(compare);
    notifyListeners();
  }
  ...
  Future<List<Category>> getAllCategory ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCategories = List.from(_allCategories); //Initialise filtered list with all data
        notifyListeners();
  ...
  void filterCategories(String keyword) {
    if (keyword.isEmpty) {
      ...
      _filteredCategories = _allCategories.where((category) {
        final nameMatch = (category.name ?? '').toLowerCase().contains(lowerKeyword);
        final dateMatch = (category.createdAt ?? '').toLowerCase().contains(lowerKeyword);
  ...
  Future<List<SubCategory>> getAllSubCategory ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredSubCategories = List.from(_allSubCategories); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<List<Brand>> getAllBrands ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredBrands = List.from(_allBrands); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  void filterBrands (String keyword) {
    if(keyword.isEmpty){
      ...
      final lowerKeyword = keyword.toLowerCase();
      _filteredBrands = _allBrands.where((subCategory) {
  ...
  Future<List<VariantType>> getAllVariantType ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredVariantTypes = List.from(_allVariantTypes); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<List<Variant>> getAllVariant ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredVariants = List.from(_allVariants); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<List<Product>> getAllProducts ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredProducts = List.from(_allProducts); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<List<Coupon>> getAllCoupons ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredCoupons = List.from(_allCoupons); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
  ...
  Future<List<Poster>> getAllPoster ({bool showSnack = false}) async {
    try {
      ...
      if (response.isOk) {
        ...
        _filteredPosters = List.from(_allPosters); //Initialise filtered list with all data
        notifyListeners();
        if (showSnack) SnackBarHelper.showSuccessSnackBar(apiResponse.message);
      }
    } catch (e) {
